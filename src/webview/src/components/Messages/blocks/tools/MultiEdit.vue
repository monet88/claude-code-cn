...
watch(
  ...
  () => {
    ...
    // Priority use toolUseResult structuredPatch (true diff returned after execution)
    if (props.toolUseResult?.structuredPatch) {
      structuredPatch.value = props.toolUseResult.structuredPatch;
    }
    // If there is input, generate temporary diff (for permission request or after real-time conversation execution)
...
// Determine if it is a permission request stage (temporary diff from input)
...
<style scoped>
/* Remove left border and margin when diff view exists, error content uses default style */
