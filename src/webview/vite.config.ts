...
export default defineConfig(({ mode }) => ({
  ...
  server: {
    ...
    strictPort: true,
    fs: {
      // Allow reading files from workspace root and external resources directory (for alias resources and icons directory)
      allow: [
  ...
  plugins: [
    ...
    }),
    {
      ...
      generateBundle(options, bundle) {
        // Only keep MDI fonts in woff2 format, delete other formats
  ...
  resolve: {
    alias: {
      ...
      // Replace resources in dependency packages with local codicon resources
